...
export function getAvailableLocales(): string[] {
  try {
    ...
    return ['en', 'es', 'de', 'pl'] // fallback to default locales
  }
}
...
export const getBlogPostMeta = cache((locale: string, slug: string): BlogPostMeta | null => {
  try {
    ...
    console.warn(`Could not read blog post ${slug} for locale ${locale}:`, error)
    return null
  }
})

// Get full blog post with content
export const getBlogPost = cache((locale: string, slug: string): BlogPost | null => {
  try {
    const fullPath = path.join(contentDirectory, locale, `${slug}.mdx`)
    const fileContents = fs.readFileSync(fullPath, 'utf8')
