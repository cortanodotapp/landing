...
import { getEmailTranslations, type SupportedLocale } from "./email-translations";
...
// Email configuration
const DEFAULT_FROM_EMAIL = process.env.SES_FROM_EMAIL || "noreply@cortano.app";
const DEFAULT_FROM_NAME = process.env.SES_FROM_NAME || "Cortano";
...
export async function sendWelcomeEmail(
  ...
  try {
    ...
    const emailHtml = await generateWelcomeEmailHtml(subscriber.locale, subscriber);
...
export function getEmailSubjects(locale: SupportedLocale = "en") {
  const subjects = {
    en: {
      welcome: "Welcome to Cortano - Your AI Call Agent Platform",
      newsletter: "Cortano Newsletter",
      unsubscribe: "You've been unsubscribed",
    },
    de: {
      welcome: "Willkommen bei Cortano - Ihre KI-Call-Agent-Plattform",
      newsletter: "Cortano Newsletter",
      unsubscribe: "Sie wurden abgemeldet",
    },
    es: {
      welcome: "Bienvenido a Cortano - Tu Plataforma de Agentes de Llamadas IA",
      newsletter: "Boletín de Cortano",
      unsubscribe: "Te has dado de baja",
    },
    pl: {
      welcome: "Witamy w Cortano - Twoja Platforma Agentów Połączeń AI",
      newsletter: "Newsletter Cortano",
